    ...
    .map(
      ([title, text], i) => `
        <article class="sub-panel">
          <strong><span class="led ${getLedClass(sectionIndex + i)}"></span>${title}</strong>
          <span>${text}</span>
        </article>
      `
    )
...
function renderBody(body = []) {
  ...
  return body
    .map(
      (paragraph, index) =>
        `<p class="${index === 0 ? "section-copy dropcap" : "section-copy"}">${paragraph}</p>`
    )
    .join("");
}
...
function renderList(items = []) {
  ...
  return `<ul class="indicator-list">${items.map((item) => `<li>${item}</li>`).join("")}</ul>`;
}
...
function renderQuotes(quotes = []) {
  ...
  return `<div class="quote-recess">${quotes
    .map(
      ([label, text]) =>
        `<blockquote class="quote-block"><span class="quote-tag">${label}</span><p>${text}</p></blockquote>`
    )
    .join("")}</div>`;
}

function renderSection(section, index) {
  const panelClass = getPanelClass(section.panelType);
  return `
    <section class="section" id="${section.id}">
      <article class="${panelClass}">
        <div class="panel-label">
          <span class="led ${getLedClass(index)}"></span>
          ${section.volume} // ${section.kicker.toUpperCase()}
        </div>
        ...
        <div class="section-body">
          ...
          ${
            section.note
              ? `<div class="callout-panel"><strong><span class="led led--amber"></span> NOTE</strong><p>${section.note}</p></div>`
              : ""
          }
        </div>
      </article>
...
document.addEventListener("mousedown", (e) => {
  if (e.target.closest(".sk-button") || e.target.closest(".knob-link")) {
    e.target.closest(".sk-button, .knob-link").style.transform = "translateY(2px)";
  }
});

document.addEventListener("mouseup", (e) => {
  if (e.target.closest(".sk-button") || e.target.closest(".knob-link")) {
    e.target.closest(".sk-button, .knob-link").style.transform = "";
  }
});
